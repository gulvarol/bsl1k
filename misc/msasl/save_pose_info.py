...
def viz_body(
    openpose_output, videofile, save_path="/users/gul/bgfs/visuals/openpose_msasl/"
):
    # Plot joints with their corresponding index
    import cv2
    import matplotlib.pyplot as plt

    fig, ax = plt.subplots()
    cap = cv2.VideoCapture(videofile)
    meta_keys = ["body", "lhnd", "rhnd", "face"]
    for fix in range(len(openpose_output)):
        ret, frame = cap.read()
        plt.imshow(frame[:, :, ::-1])
        for m in meta_keys:
            for pt in range(openpose_output[fix][m].shape[0]):
                x = openpose_output[fix][m][pt, 0]
                y = openpose_output[fix][m][pt, 1]
                if x != 0 and y != 0:
                    ax.scatter(x, y)
                    ax.annotate(f"{pt}", (x, y))
        plt.savefig(f"{save_path}/img{fix:03d}.png")
        ax.cla()
...
def get_openpose_video(openpose_dir, num_frames):
    ...
    if num_frames != len(json_files):
        print(f"Num frames mismatch {openpose_dir}")
        return None
...
def main(dataset_path):
    ...
    for ix in tqdm(range(N)):
        ...
        openpose_dir = os.path.join(
            "/users/gul/datasets/mslasl/mslasl/", split, videoname
        )
        ...
        if openpose_data is None:
            ...
            # viz_body(openpose_data, videolist[ix])
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--dataset_path",
        type=str,
        default="/users/gul/datasets/msasl",
